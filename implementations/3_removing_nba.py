...
def get_adj(dataset_name):
    ...
    # idx = np.arange(idx_features_labels["user_id"])
    # idx_map = {j: i for i, j in enumerate(idx)}
    # edges = np.array(list(map(idx_map.get, edges_unordered.flatten())),
    #                  dtype=int).reshape(edges_unordered.shape)
    # print("idx with user_id: ", idx)
    # print("idx with [0]", np.array(idx_features_labels[0]))
...
max_num = 0
print("edit: ",len(final_influence[harmful_idx])) # 6
print("final_influence: ", final_influence)
print("index 0: ", final_influence[harmful_idx][0]) # nan
print("index 1: ", final_influence[harmful_idx][0+1]) # nan
...
if dataset_name == 'bail':
    ...
    adj_ori, features_ori, labels_ori, idx_train_ori, idx_val_ori, idx_test_ori, sens_ori = load_nba('nba')
    norm_features_ori = feature_norm(features_ori)
    norm_features_ori[:, 0] = features_ori[:, 0]
    features_ori = feature_norm(features_ori)
