...
if dataset_name == 'nba':
  # /content/GNN_Fairness_Proj/gcn_nba.pth
    # print("MODEL TORCH.LOAD: ", torch.load)
  #  model = torch.load('gcn_' + dataset_name + '.pth')

    model = torch.load('gcn_' + dataset_name + '.pth')
    adj_vanilla, features_vanilla, labels_vanilla, idx_train_vanilla, idx_val_vanilla, idx_test_vanilla, sens_vanilla = load_nba(dataset_name)
...
for i in tqdm(range(idx_train_vanilla.shape[0])):
    ...
    sens = sens.to(torch.device('cuda'))
...
print("len(non_iid_influence:", len(non_iid_influence))

final_influence = []
for i in range(len(non_iid_influence)):
    print("computation_graph_involving[i]", computation_graph_involving[i]) 
    print("[idx_train_vanilla[i]]", [idx_train_vanilla[i]])
    # for item in (computation_graph_involving[i] + [idx_train_vanilla[i]]): 

    ref = [idx_train_vanilla.numpy().tolist().index(item) for item in (computation_graph_involving[i] + [idx_train_vanilla[i]])]
    # print("REF: ", ref)
    print("non_iid_influence[i]", non_iid_influence[i])
    # print("np.array(influence)[ref].sum()", np.array(influence)[ref].sum())
...
# if dataset_name == 'nba': 
#     model = torch.load('gcn_' + dataset_name + '.pth')
#     adj_vanilla, features_vanilla, labels_vanilla, idx_train_vanilla, idx_val_vanilla, idx_test_vanilla, sens_vanilla = load_nba('nba')
#     # norm_features = feature_norm(features_vanilla)
#     # norm_features[:, 0] = features_vanilla[:, 0]
#     # features_vanilla = norm_features
