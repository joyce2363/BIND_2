...
def part4(dataset_name, seed):
    ...
    if dataset_name == 'income':
        ...
        final_sets = np.load('1final_sets_' + str(args.model) + dataset_name + str(seed) + '.npy', allow_pickle=True).item()
    print("LENGTH OF HARMFUL NODES in ACC_RECORD: ", len(final_sets['acc_records']))
    print("LENGTH OF HARMFUL NODES in SP_RECORD: ", len(final_sets['sp_records']))
    print("LENGTH OF HARMFUL NODES in EO_RECORD: ", len(final_sets['eo_records']))
    ...
    # threshold10=min(len(final_sets['acc_records']),thenumberofnodes*0.1)

    if(len(final_sets['acc_records']) > 100): 
        print("BIND 10%:")
        budget = 100
        print("Acc:", final_sets['acc_records'][budget])
